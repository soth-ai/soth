...
export type SourceStreamVariant = "ai_proxy" | "agent_app";

const PREVIEW_DECODE_LIMIT = 4096;

function filterLogsByVariant(
  logs: LogEntry[],
  filters: Filters,
  variant: SourceStreamVariant
): LogEntry[] {
  return logs.filter((log) => {
    if (log.source !== variant) {
      return false;
    }
    ...
    if (filters.searchText) {
      ...
      } else if (!matchesContent && !matchesProvider && !matchesModel && !matchesMethod) {
        return false;
      }
    }
    ...
    if (filters.serverName) {
      ...
      if (!providerMatches && !hostMatches) {
        return false;
      }
    }
    ...
    if (filters.method && log.model !== filters.method) {
      return false;
    }
...
interface SourceLogRowProps {
  log: LogEntry;
  variant: SourceStreamVariant;
}

const SourceLogRow = memo(function SourceLogRow({
  log,
  variant,
}: SourceLogRowProps) {
  ...
  const isAgentVariant = variant === "agent_app";
  const selectedRingClass = isAgentVariant
  ...
  const agentName = useMemo(() => getAgentName(log), [log]);
      ...
      <div
        ...
        className={cn(
          ...
          isSelected
            ...
            : isAgentVariant
            ? "border-l-muted-foreground/30 hover:border-l-purple-500/50 hover:bg-muted/20"
            : "border-l-muted-foreground/30 hover:border-l-accent/50 hover:bg-muted/20"
        )}
      ...
      >
        ...
        >
          <div className="flex-shrink-0">
            <div className={cn("w-2 h-2 rounded-full bg-cyan-500", requestIsSelected && selectedRingClass)} />
          </div>
          ...
          {isAgentVariant ? (
            ...
            log.provider && (
              <span
                className={cn(
                  "inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-mono font-medium flex-shrink-0 min-w-[70px] max-w-[80px] truncate border",
          ...
          <div className="flex-shrink-0">
            <div
              className={cn(
                ...
                responseIsSelected && selectedRingClass
              )}
            />
          </div>
          ...
          {isHovered && (
            <Button
              ...
              onClick={(e) =>
                handleCopyJson(
                  e,
                  log.response_content || log.response_preview || responsePreview || ""
                )
              }
              className="h-6 w-6 p-0 bg-secondary border border-border hover:bg-muted"
              title="Copy response"
  ...
  const getStatusColor = () => {
    ...
    return isAgentVariant ? "bg-purple-500" : "bg-emerald-500";
  };
    ...
    >
      ...
      <div className="flex-shrink-0">
        {log.direction === "in" ? (
          ...
          <ArrowDown
            className={cn("w-3 h-3", isAgentVariant ? "text-purple-500" : "text-emerald-500")}
            weight="bold"
          />
        )}
      </div>
      ...
      {!isAgentVariant && log.cost_usd !== undefined && log.cost_usd > 0 && (
        <span
          className="text-[10px] font-mono flex-shrink-0 px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-500 tabular-nums"
          title={`$${log.cost_usd.toFixed(6)}`}
        >
...
export function SourceInferenceStream({
  variant,
}: {
  variant: SourceStreamVariant;
}) {
  ...
  const filteredLogs = useMemo(
    () => filterLogsByVariant(logs, filters, variant),
    [logs, filters, variant]
  );
  const allVariantLogs = useMemo(
    () => logs.filter((log) => log.source === variant),
    [logs, variant]
  );
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card">
        <div className="flex items-center gap-3">
          <h2 className="text-sm font-semibold text-foreground">
            {isAgentVariant ? "Agent Apps Stream" : "AI Inference Stream"}
          </h2>
          <span className="text-[10px] font-semibold text-muted-foreground bg-muted px-1.5 py-0.5 rounded-md border border-dashed border-border uppercase tracking-wide">
            {filteredLogs.length}
            {hasActiveFilters && ` / ${allVariantLogs.length}`}
          </span>
            ...
            >
              <div
                className={cn(
                  ...
                  isAgentVariant ? "bg-purple-500" : "bg-emerald-500"
                )}
              />
              <span
                className={cn(
                  "text-[10px] font-semibold tracking-wide",
                  isAgentVariant ? "text-purple-500" : "text-emerald-500"
                )}
              >
                LIVE
              </span>
        ...
        {filteredLogs.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-sm">
              {hasActiveFilters && allVariantLogs.length > 0 ? (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    {allVariantLogs.length} request{allVariantLogs.length === 1 ? "" : "s"} hidden by
                    filters.
                  </p>
                  <Button
                    variant="outline"
                ...
                <>
                  <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-accent/20 to-accent/5 flex items-center justify-center">
                    <CloudArrowUp className="w-8 h-8 text-accent" weight="duotone" />
                  </div>
                  <p className="text-base text-foreground font-semibold mb-2">
          ...
          <Virtuoso
            ...
            itemContent={(index, log) => (
              <SourceLogRow log={log} variant={variant} />
            )}
...
export function AiInferenceStream() {
  return <SourceInferenceStream variant="ai_proxy" />;
}
