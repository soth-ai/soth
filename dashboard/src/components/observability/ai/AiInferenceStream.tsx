...
// Helper function to filter AI proxy logs
function filterAiLogs(logs: LogEntry[], filters: Filters): LogEntry[] {
  return logs.filter((log) => {
    // Only AI proxy logs
    if (log.source !== "ai_proxy") return false;
    ...
    if (filters.searchText) {
      ...
      if (!matchesContent && !matchesProvider && !matchesModel && !matchesMethod) {
        return false;
      }
    }
    ...
    >
      ...
      <div className="flex-shrink-0">
        {log.direction === "in" ? (
          ...
          <ArrowDown className="w-3 h-3 text-emerald-500" weight="bold" />
        )}
      </div>
...
export function AiInferenceStream() {
  ...
  const filteredLogs = useMemo(() => filterAiLogs(logs, filters), [logs, filters]);
  const allAiLogs = useMemo(() => logs.filter((l) => l.source === "ai_proxy"), [logs]);
        ...
        {filteredLogs.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-sm">
              {hasActiveFilters && allAiLogs.length > 0 ? (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    {allAiLogs.length} request{allAiLogs.length === 1 ? "" : "s"} hidden by
                    filters.
                  </p>
                  <Button
                    variant="outline"
          ...
          <Virtuoso
            ref={virtuosoRef}
            data={filteredLogs}
            itemContent={(index, log) => <AiLogRow key={log.id} log={log} index={index} />}
