...
pub(super) fn compute_merkle_root(leaves: &[[u8; 32]]) -> [u8; 32] {
    if leaves.is_empty() {
...
pub(super) fn hash_root_link(prev_root: &[u8; 32], current_root: &[u8; 32]) -> [u8; 32] {
    let mut hasher = Sha256::new();
...
pub(super) fn hex_decode_32(value: &str) -> std::io::Result<[u8; 32]> {
...
pub(super) fn to_io_err(error: rusqlite::Error) -> std::io::Error {
