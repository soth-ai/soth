//! Bundle-format-driven usage and token extraction helpers.
...
pub fn extract_request_pii_probe(
    oisp_engine: Option<&OispEngine>,
    provider: &str,
    host: &str,
    decoded_body: &[u8],
...
pub async fn extract_usage_meta(
    oisp_engine: Option<&OispEngine>,
    provider: &str,
    host: &str,
    decoded_body: &[u8],
...
mod tests {
    ...
    fn test_oisp_engine() -> OispEngine {
        ...
        std::fs::write(
            path.clone(),
            serde_json::to_vec_pretty(&envelope).expect("serialize"),
        )
        .expect("write");
