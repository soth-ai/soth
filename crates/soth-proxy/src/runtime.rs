...
//! and passes it in. Passing an empty registry disables extensions.
...
use crate::{bundle_runtime, config::ProxyConfig, db, ProxyHandler};
...
/// Initialise the default rustls crypto provider if one hasn't been installed.
...
///
/// Caller must have initialised rustls + tracing beforehand (via
/// [`init_rustls_provider`] and [`init_tracing`]) so that the extension
...
#[cfg(feature = "extensions")]
async fn run_inner(ext_registry: Option<ExtensionRegistry>) -> Result<()> {
    let ext_registry = ext_registry.unwrap_or_else(ExtensionRegistry::empty);
        ...
        tokio::spawn(async move {
            ...
            match tokio::net::TcpListener::bind(&ops_bind).await {
                Ok(listener) => {
                    ...
                    if let Err(e) =
                        axum::serve(listener, crate::ops_server::build_router(ops_state)).await
                    {
...
fn inherited_listener() -> Result<Option<tokio::net::TcpListener>> {
    ...
    // SAFETY: the supervisor (`soth start`) passes an open, owned file
    // descriptor via SOTH_LISTENER_FD when spawning the worker and does not
    // touch it again. Taking ownership here is sound because the supervisor
    // has cleared FD_CLOEXEC and stopped using the fd before exec.
...
async fn ensure_classify_models(config: &crate::config::ProxyConfig) {
