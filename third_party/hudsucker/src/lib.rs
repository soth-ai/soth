...
/// TLS failure stage observed in CONNECT handling.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TlsFailureStage {
    /// Reading initial bytes from upgraded client stream failed.
    ClientHelloRead,
    /// Upstream certificate sniffing failed.
    UpstreamCertSniff,
    /// TLS handshake with client failed.
    ClientHandshake,
    /// TCP tunnel connect to upstream failed.
    UpstreamConnect,
    /// Tunnel copy between client and upstream failed.
    TunnelCopy,
}
...
pub trait HttpHandler: Clone + Send + Sync + 'static {
    ...
    /// Called when CONNECT-stage TLS/tunnel failures are observed.
    fn handle_tls_failure(
        &mut self,
        _ctx: &HttpContext,
        _authority: Option<&Authority>,
        _stage: TlsFailureStage,
        _error: &str,
