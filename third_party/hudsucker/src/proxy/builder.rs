...
impl<CA, C, H, W, F> ProxyBuilder<WantsHandlers<CA, C, H, W, F>> {
    ...
    pub fn with_http_handler<H2: HttpHandler>(
        ...
        ProxyBuilder(WantsHandlers {
            ...
            upstream_cert_sniffing: self.0.upstream_cert_sniffing,
    ...
    /// Enable or disable upstream certificate sniffing before generating MITM certs.
    pub fn with_upstream_cert_sniffing(self, enabled: bool) -> Self {
        ProxyBuilder(WantsHandlers {
            upstream_cert_sniffing: enabled,
            ..self.0
        })
    }
